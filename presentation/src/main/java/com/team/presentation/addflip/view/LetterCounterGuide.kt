...
fun LetterCounterGuide(
    ...
    val color =
        when {
            ...
            progress in FirstStepStart..FirstStepEnd -> {
                FlipLightColors.statusBlue
            }
            progress in SecondStepStart..SecondStepEnd -> {
                FlipLightColors.statusBlue
            }
            ThirdStepStart <= progress && progress < ThirdStepEnd -> {
                FlipLightColors.point
            }
            else -> {
                /** progress >= 1f */
    ...
    val guideline =
        when {
            ZeroStep >= progress -> {
                R.string.add_flip_screen_letter_guide_notice_1
            }
            progress in FirstStepStart..FirstStepEnd -> {
                R.string.add_flip_screen_letter_guide_notice_2
            }
            progress in SecondStepStart..SecondStepEnd -> {
                R.string.add_flip_screen_letter_guide_notice_3
            }
            ThirdStepStart <= progress && progress < ThirdStepEnd -> {
                R.string.add_flip_screen_letter_guide_notice_4
            }
            else -> {
                /** progress >= 1f */
    ...
    val guideline2 =
        when {
            ZeroStep >= progress -> {
                R.string.add_flip_screen_letter_guide_notice_1_2
            }
            progress in FirstStepStart..FirstStepEnd -> {
                R.string.add_flip_screen_letter_guide_notice_2_2
            }
            progress in SecondStepStart..SecondStepEnd -> {
                R.string.add_flip_screen_letter_guide_notice_3_2
            }
            ThirdStepStart <= progress && progress < ThirdStepEnd -> {
                R.string.add_flip_screen_letter_guide_notice_4_2
            }
            else -> {
                /** progress >= 1f */
    ...
    Column(
        modifier = modifier.fillMaxWidth().background(FlipTheme.colors.gray1).padding(16.dp),
        verticalArrangement = Arrangement.spacedBy(4.dp),
    ) {
        TitleBar(modifier = Modifier.fillMaxWidth().padding(bottom = 8.dp), length, limit, color)
        LetterProgressBar(modifier = Modifier.fillMaxWidth(), color = color, progress = progress)
        NoticeBar(
...
@Composable
private fun TitleBar(modifier: Modifier = Modifier, length: Float, limit: Float, color: Color) {
...
@Composable
private fun LetterProgressBar(modifier: Modifier = Modifier, color: Color, progress: Float) {
    ...
    Box(modifier = modifier.height(4.dp)) {
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            ...
            drawCircle(
                color =
                    if (progress >= 1f || progress >= ZeroStep) color else FlipLightColors.gray1,
                radius = 4.dp.toPx(),
                center = Offset(0f + 2.dp.toPx(), size.height / 2),
            )
            drawCircle(
                color =
                    if (progress >= 1f || progress >= FirstStepEnd) color
                    else FlipLightColors.gray1,
                radius = 4.dp.toPx(),
                center = Offset(size.width / 3, size.height / 2),
            )
            drawCircle(
                color =
                    if (progress >= 1f || progress >= SecondStepEnd) color
                    else FlipLightColors.gray1,
                radius = 4.dp.toPx(),
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            ...
            drawCircle(color = color, center = Offset(size.width * progress, size.height / 2))
        }
    }
}
...
fun LetterCounterGuidePreview() {
    ...
    FlipAppTheme {
        Box(modifier = Modifier) {
            LetterCounterGuide(length = 130f, limit = 300f, progress = 0.5f)
        }
    }
}
