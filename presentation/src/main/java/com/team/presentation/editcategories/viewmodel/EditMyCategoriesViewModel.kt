...
@HiltViewModel
class EditMyCategoriesViewModel
@Inject
constructor(
     ...
     *
     * SPEECH_BUBBLE_COUNT_LIMIT가 0이면 1번 표시하는 이유는 최초의 count 값이 null이기 때문
     */
            ...
            try {
                ...
                when (myProfileResult) {
                    ...
                    is Result.Success -> {
                        ...
                        if (myCategoryIds != null) {
                            exclusiveCategories =
                                categories.filter { !myCategoryIds.contains(it.id) }
                            myCategories =
                                myCategoryIds.mapNotNull { id -> categories.find { it.id == id } }
                            ...
                            _myCategoriesState.update {
                                it.copy(loading = false, error = ErrorType.Local.EMPTY.asUiText())
                            }
                        }
                    }
                ...
                _myCategoriesState.update {
                    it.copy(
                        loading = false,
                        error =
                            e.localizedMessage?.let { error -> UiText.DynamicString(error) }
    ...
    fun onUiEvent(uiEvent: EditMyCategoriesUiEvent) {
        ...
        when (uiEvent) {
            ...
            is EditMyCategoriesUiEvent.SelectCategory -> {
                ...
                exclusiveCategories =
                    exclusiveCategories.toMutableList().apply { remove(uiEvent.category) }
            }
            is EditMyCategoriesUiEvent.UnSelectCategory -> {
                myCategories = myCategories.toMutableList().apply { remove(uiEvent.category) }
                exclusiveCategories =
                    exclusiveCategories.toMutableList().apply { add(uiEvent.category) }
            }
        ...
        _myCategoriesState.update {
            it.copy(myCategories = myCategories, exclusiveCategories = exclusiveCategories)
        }
    }
    ...
    fun updateMyCategories(reorderedCategories: List<Category>) {
        ...
        if (reorderedCategories != myCategoriesCache) {
            ...
            updateMyCategoriesUseCase(reorderedCategories)
                .onEach { result ->
                    when (result) {
                        Result.Loading -> {
            ...
            _myCategoriesUpdateState.update { it.copy(success = true) }
        }
    }
}
