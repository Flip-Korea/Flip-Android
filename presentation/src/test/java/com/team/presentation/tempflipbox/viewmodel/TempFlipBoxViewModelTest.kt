...
class TempFlipBoxViewModelTest {
    ...
    @Test
    fun `임시저장플립 삭제 실패 (deleteTempPosts())`() =
        runTest {
            // Given
            val actualErrorBody = ErrorBody("", emptyList(), "error")
            val tempPostId: Long = 1
            every { getTempPostsUseCase() } returns flowOf(PagingData.empty())
            every { deleteTempPostUseCase(tempPostId) } returns
                flowOf(
                    Result.Error(
            ...
            // Then
            assertEquals(snackbarEvent?.message, UiText.DynamicString(actualErrorBody.message))
        }

    @Test
    fun `임시저장플립 삭제 성공 (deleteTempPosts())`() =
        runTest {
            // Given
            val tempPostId: Long = 1
            every { getTempPostsUseCase() } returns flowOf(PagingData.empty())
            every { deleteTempPostUseCase(tempPostId) } returns flowOf(Result.Success(true))
