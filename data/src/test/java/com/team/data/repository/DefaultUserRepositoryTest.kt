...
class DefaultUserRepositoryTest {
    ...
    @Test
    fun `내 프로필 새로고침 (getMyProfileRefresh())`() =
        runTest {
    ...
    @Test fun `내 프로필 조회(Local 작업) (getMyProfile())`() = runTest {}
    ...
    @Test
    fun `카테고리 업데이트 (updateMyCategories())`() =
        runTest {
            ...
            val response = userRepository.updateMyCategories(categoryIds = categories).last()
    ...
    fun `계정 차단 (blockAccount())`() =
        runTest {
            ...
            val blockReq = BlockReq(profileId = "honggd", blockedId = "bad_friend", postId = 123L)
    ...
    fun `계정 차단 해제 (unblockAccount())`() =
        runTest {
            ...
            val result = userRepository.unblockAccount("honggd", "bad_friend").last()
    ...
    fun `본인 프로필 수정 (editMyProfile())`() =
        runTest {
            ...
            val result = userRepository.editMyProfile(profileId, editProfile).last()
    ...
    @Test
    fun `팔로워 목록 조회 페이지네이션 (getFollowerListPagination())`() =
        runTest {
            server.enqueue(
                MockResponse().apply {
                    setResponseCode(200)
    ...
    @Test
    fun `팔로워 목록 조회 페이지네이션(hasNext X) (getFollowerListPagination())`() =
        runTest {
            server.enqueue(
                MockResponse().apply {
                    setResponseCode(200)
    ...
    @Test
    fun `팔로잉 목록 조회 페이지네이션 (getFollowingListPagination())`() =
        runTest {
            server.enqueue(
                MockResponse().apply {
                    setResponseCode(200)
    ...
    @Test
    fun `팔로잉 목록 조회 페이지네이션(hasNext X) (getFollowingListPagination())`() =
        runTest {
            server.enqueue(
                MockResponse().apply {
                    setResponseCode(200)
    ...
    @Test
    fun `차단 목록 조회 페이지네이션 (getBlockListPagination())`() =
        runTest {
            server.enqueue(
                MockResponse().apply {
                    setResponseCode(200)
    ...
    @Test
    fun `차단 목록 조회 페이지네이션(hasNext X) (getBlockListPagination())`() =
        runTest {
            server.enqueue(
                MockResponse().apply {
                    setResponseCode(200)
    ...
    @Test
    fun `내 댓글 목록 조회 페이지네이션 (getMyCommentListPagination())`() =
        runTest {
            server.enqueue(
                MockResponse().apply {
                    setResponseCode(200)
    ...
    @Test
    fun `내 댓글 목록 조회 페이지네이션(hasNext X) (getMyCommentListPagination())`() =
        runTest {
            server.enqueue(
                MockResponse().apply {
                    setResponseCode(200)
