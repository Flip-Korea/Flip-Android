...
class UserNetworkDataSourceTest {
    ...
    @Test
    fun `카테고리 관심분야 선택(selectMyCategory())`() =
        runTest {
    ...
    fun `스크랩 목록 불러오기 (getScrapList())`() =
        runTest {
            ...
            val actualResponse =
                userNetworkDataSource.getScrapList(
                    profileId = "honggd",
                    cursor = "test-cursor",
                    limit = 15,
                )
    ...
    fun `스크랩 추가 (addScrap())`() =
        runTest {
            ...
            val actualResponse =
                userNetworkDataSource.addScrap(requestAdapter.fromJson(addScrapRequestTestData)!!)
    ...
    @Test
    fun `계정 차단 (blockAccount())`() =
        runTest {
            server.enqueue(MockResponse().apply { setResponseCode(201) })

            val blockRequest =
                BlockRequest(profileId = "honggd", blockedId = "bad_friend", postId = 123L)
    ...
    @Test
    fun `계정 차단 해제 (unblockAccount())`() =
        runTest {
            server.enqueue(MockResponse().apply { setResponseCode(201) })

            val result = userNetworkDataSource.unblockAccount("honggd", "bad_friend")

            assert((result as Result.Success).data)
        }

    @Test
    fun `본인 프로필 수정 (editMyProfile())`() =
        runTest {
            server.enqueue(MockResponse().apply { setResponseCode(201) })

            val editProfileRequest =
                EditProfileRequest(nickname = "nickname", introduce = "introduce", photoUrl = "asd")
    ...
    fun `팔로워 목록 조회 (getFollowerList())`() =
        runTest {
            ...
            val expectedResponse =
                moshi.adapter(FollowerListResponse::class.java).fromJson(networkFollowersTestData)
    ...
    @Test
    fun `팔로잉 목록 조회 (getFollowingList())`() =
        runTest {
            server.enqueue(
                MockResponse().apply {
                    setResponseCode(200)
            ...
            val expectedResponse =
                moshi.adapter(FollowingListResponse::class.java).fromJson(networkFollowingsTestData)
    ...
    fun `차단 목록 조회 (getBlockList())`() =
        runTest {
            ...
            val expectedResponse =
                moshi.adapter(BlockListResponse::class.java).fromJson(networkBlocksTestData)
    ...
    @Test
    fun `내 댓글 목록 조회 (getMyCommentList())`() =
        runTest {
            server.enqueue(
                MockResponse().apply {
                    setResponseCode(200)
            ...
            val expectedResponse =
                moshi.adapter(MyCommentListResponse::class.java).fromJson(networkMyCommentsTestData)
    ...
    @Test
    fun `내 카테고리 관심분야 조회 (getMyCategories())`() =
        runTest {
            server.enqueue(
                MockResponse().apply {
                    setResponseCode(200)
