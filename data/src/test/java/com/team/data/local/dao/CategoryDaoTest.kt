...
import com.team.data.local.testdoubles.makeCategoriesTestData
import com.team.data.local.testdoubles.makeCategoryTestData
...
class CategoryDaoTest {
    ...
    @Test
    fun `모든 카테고리 가져오기 (getCategories())`() =
        runTest {
    ...
    @Test
    fun `ID로 카테고리 가져오기 (getCategoryById())`() =
        runTest {
    ...
    @Test
    fun `ID로 카테고리 여러 개 가져오기 (getCategoryById())`() =
        runTest {
            val categoryIds = mutableListOf<Int>()
    ...
    @Test
    fun `존재하지 않은 ID로 카테고리 가져오기 (getCategoryById())`() =
        runTest {
            categoryDao.upsertCategory(makeCategoryTestData(1))
