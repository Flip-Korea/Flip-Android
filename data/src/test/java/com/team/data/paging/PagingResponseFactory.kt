...
class PagingResponseFactory {
    ...
    private fun createPagingResponse(): FakePagingResponse {
        ...
        return FakePagingResponse(id = id.toLong(), content = "#$id content")
    }

    fun createPagingListResponse(limit: Int): FakePagingListResponse {
        val list = List(limit) { createPagingResponse() }
        return FakePagingListResponse(list = list, listTotal = 100L)
    }
}
