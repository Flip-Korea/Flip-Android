...
class FlipPagingSource<T : Any, R : FlipPagingData<T>>(
    ...
    override fun getRefreshKey(state: PagingState<Long, T>): Long? =
        state.anchorPosition?.let { anchorPosition ->
            ...
            //            val refreshKey = anchorPage?.prevKey?.plus(1) ?:
    ...
    override suspend fun load(params: LoadParams<Long>): LoadResult<Long, T> {
        try {
             ...
             *
             * [result]가 [Result.Error]일 경우는 위에서 얼리리턴 될 것 이고, [apiCall]에서는 [networkCall]호출을 하기 때문에
             * [Result.Loading]을 반환하는 경우는 없다. 그러므로, [Result.Error]가 아닌 경우는 무조건 [Result.Success]를
             * 반환하는 경우이다.
             */
